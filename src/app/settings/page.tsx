...
export default function SettingsPage() {
  ...
  const previewUrl = `https://casalead.de/widgets/valuation.html?company=${encodeURIComponent(config.company)}&farbe=${encodeURIComponent(config.farbe)}&farbe_dunkel=${encodeURIComponent(config.farbe_dunkel)}&farbe_hell=${encodeURIComponent(config.farbe_hell)}&border_radius=${encodeURIComponent(config.border_radius)}&input_border_radius=${encodeURIComponent(config.input_border_radius)}&schriftart=${encodeURIComponent(config.schriftart)}`

  const embedCode = `<script src=\"https://casalead.de/widgets/valuation.js\" defer></script>\n<casalead-widget company=\"${config.company}\" farbe=\"${config.farbe}\" farbe_dunkel=\"${config.farbe_dunkel}\" farbe_hell=\"${config.farbe_hell}\" border_radius=\"${config.border_radius}\" input_border_radius=\"${config.input_border_radius}\" schriftart=\"${config.schriftart}\"></casalead-widget>`
    ...
    <div className="flex h-screen bg-gray-100 font-inter text-gray-800">
      ...
      <main className="flex-1 overflow-auto relative">
        <header className="bg-white shadow-sm flex justify-between items-center px-6 py-4">
          <h2 className="text-2xl font-semibold text-gray-900">Widget Generator</h2>
        </header>
        ...
        <section className="p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white rounded-xl shadow p-6">
            <div className="space-y-6">
              <div>
                <label className="block font-medium text-gray-700">Firmenname</label>
                <input type="text" value={config.company} onChange={(e) => updateField('company', e.target.value)} className="w-full p-2 rounded border border-gray-300" />
              </div>
              <div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <textarea value={embedCode} readOnly rows={5} className="w-full font-mono bg-gray-100 border border-gray-300 p-4 rounded text-gray-800" />
